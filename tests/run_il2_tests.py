...
def run_tests(test_filter=None, verbose=False, coverage=False):
    ...
    if coverage:
        cmd.extend(
            [
                "--cov=il2_sturmovik_plugin.il2_parser",
                "--cov-report=html",
                "--cov-report=term",
            ]
        )
...
def main():
    ...
    parser.add_argument(
        "--encoding-only", action="store_true", help="Run only encoding-related tests"
    )
    ...
    # Run tests
    exit_code = run_tests(
        test_filter=test_filter, verbose=args.verbose, coverage=args.coverage
    )
